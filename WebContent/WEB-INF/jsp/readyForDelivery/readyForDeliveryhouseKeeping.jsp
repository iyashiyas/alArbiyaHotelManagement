...
<li><a href="?lang=ar"><spring:message
										  ...
										  <c:forEach items="${orders}" var="orders">
												...
												<td>
												   <c:if test="${orders.status=='READYFORDELIVERY'}">
														<a href="#" id="confirmDeliveryRequest" data-toggle="modal" data-target="#confirmReadForDelivery" data-href="${pageContext.request.contextPath}/order/deliveryBoyAccepthouseKeeping?orderId=${orders.orderId}" class="btn btn-primary">
															<spring:message code="label.AcceptRequest" />
														</a> 
														</c:if> 
														 <c:if test="${orders.status=='ACCEPTREQUEST'}">
														<a href="#" id="deliverdAccept" data-toggle="modal" data-target="#confirmDelivered" data-href="${pageContext.request.contextPath}/order/deliverdAccceptHouseKeeping?id=${orders.orderId}&roomId=${orders.roomId}" class="btn btn-danger">
															 Accept Delivery 
														</a> 
														</c:if> 
														<c:if test="${orders.status=='DELIVERED'}">
														 	<label class="label label-default">CLEAN </label>
														</c:if>  
												</td>
												</tr> 
												</c:forEach>
 ...
 <script src="<c:url value="/resources/js/order/housekeepingdeliveryBoy.js"/>"></script>
	...
	<script> 
	 $('#confirmReadForDelivery').on('show.bs.modal', function(e){ 
  	...
  	$('#confirmDelivered').on('show.bs.modal', function(e){   
  		$('#confirmDelivered').find('.btn-okdelivered').attr('href', $('#deliverdAccept').data('href')); 
  		 });
